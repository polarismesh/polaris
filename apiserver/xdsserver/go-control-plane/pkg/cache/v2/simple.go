...
import (
	...
	"github.com/polarismesh/polaris-server/apiserver/xdsserver/go-control-plane/pkg/log"
)
	...
	if info, ok := cache.status[node]; ok {
		...
		for id, watch := range info.watches {
			...
			if version != watch.Request.VersionInfo {
				if cache.log != nil {
					cache.log.Debugf("respond open watch %d%v with new version %q",
						id, watch.Request.ResourceNames, version)
				}
...
// TODO(kuat) do not respond always,
// see issue https://github.com/polarismesh/polaris-server/apiserver/xdsserver/go-control-plane/issues/46
func (cache *snapshotCache) respond(request Request,
	value chan Response, resources map[string]types.Resource, version string) {
